...
@Component({
  selector: 'app-publication-create-boutique',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './publication-create-boutique.component.html',
})
export class PublicationCreateBoutiqueComponent implements OnDestroy {
  ...
  async submit(): Promise<void> {
    ...
    this.publicationService.createPublication(formData).subscribe({
      next: () => {
        this.router.navigate(['/store/app/publications'], { queryParams: { tab: 'pending' } });
      },
      error: (err) => {
  ...
  cancel(): void {
    this.router.navigate(['/store/app/publications']);
  }
